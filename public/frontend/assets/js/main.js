...
if (lookbookUnderwear) {
    fetch("./assets/data/Product.json")
        ...
        .then((products) => {
            const itemDot = lookbookUnderwear.querySelector(
                ".list-img .item .dots"
            );
            ...
            colorItems.forEach((colorItem) => {
                colorItem.addEventListener("click", () => {
                    ...
                    if (targetIndex !== -1) {
                        ...
                        if (
                            document.querySelector(".product-detail .mySwiper2")
                        )
                            swiper2.slideTo(targetIndex); // scroll slide to index
                    } else {
...
const handleActiveImgWhenColorChange = (products) => {
    ...
    listColors.forEach((list) => {
        ...
        colorItems.forEach((color) => {
            color.addEventListener("click", () => {
                const activeColor =
                    color.querySelector(".tag-action")?.textContent;
...
fetch("./assets/data/Product.json")
    ...
    .then((products) => {
                ...
                if (parent.querySelector(".menu-tab .active")) {
                    ...
                    menuItems.forEach((item) => {
                        item.addEventListener("click", () => {
                            // remove old product
                            const productItems =
                                list.querySelectorAll(".product-item");
                            productItems.forEach((prdItem) => {
                                prdItem.remove();
                            });

                            if (
                                list.getAttribute("data-type") === "underwear"
                            ) {
                                if (
                                ...
                                if (
                                    item.getAttribute("data-item") === "on sale"
                                ) {
                                    products
                                        .filter(
                                            (product) =>
        ...
        if (listSixProduct) {
            const parent =
                listSixProduct.parentElement.parentElement.parentElement;
            if (parent.querySelector(".menu-tab .active")) {
                const menuItemActive = parent
                    .querySelector(".menu-tab .active")
                    .getAttribute("data-item");
                ...
                if (listEightProduct.getAttribute("data-type")) {
                    products
                        .filter(
                            (product) =>
                                product.category ===
                                listEightProduct.getAttribute("data-type")
                        )
                        .slice(0, 8)
                        .forEach((product) => {
                            const productElement = createProductItem(product);
        ...
        .then((products) => {
            ...
            if (listProduct) {
                ...
                if (parent.querySelector(".menu-tab .active")) {
                    ...
                    products
                        .filter(
                            (product) => product.category === menuItemActive
                        )
                        .slice(0, 5)
                        .forEach((product) => {
                            const productElement =
                                createProductItemMarketplace(product);
...
const handleInforCart = () => {
    if (listProductCart) {
        ...
        cartStore.forEach((product) => {
            ...
            const quantityBlock =
                productElement.querySelector(".quantity-block");
