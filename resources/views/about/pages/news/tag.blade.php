        ...
        <div class="container">
            <div class="row align-items-xl-center">
                <div class="col-md-6" style="margin-bottom: 30px">
                    <h4 class="h5">Tags</h4>
                    <div class="tagcloudIndex">
                        <a href="#" class="active">Yoriko</a>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-8">
                    @foreach ($articles as $item)
                        <div class="vs-blog">
                            <div class="blog-image image-scale-hover"><a
                                    href="{{ route('about.newsDetail', $item->slug) }}"><img
                                        src="{{ asset('storage/' . $item->images) }}" alt="{{ $item->tittle }}"></a></div>
                            <div class="blog-content bg-light-theme">
                                <h2 class="blog-title"><a
                                        href="{{ route('about.newsDetail', $item->slug) }}">{{ $item->tittle }}</a></h2>
                                <div class="blog-meta">
                                    <i class="fal fa-calendar-alt text-theme"></i>
                                ...
                                <div class="blog-footer d-flex justify-content-between align-items-center">
                                    <a href="{{ route('about.newsDetail', $item->slug) }}" class="link-btn"><i
                                            class="fal fa-long-arrow-right mr-2 ml-0"></i>Read
                                        More</a>
                                </div>
                            </div>
                        </div>
                                ...
                                @foreach ($popularArticles as $popular)
                                    <div class="recent-post media">
                                        <div class="media-img"><img width="150" height="100"
                                                src="{{ asset('storage/' . $popular->images) }}"
                                                alt="{{ $popular->tittle }}"></div>
                                        <div class="media-body pl-20">
                                            <h4 class="recent-post-title h6 mb-0"><a
                                                    href="{{ route('about.newsDetail', $popular->slug) }}">{{ $popular->tittle }}</a>
                                            </h4><span><i class="fal fa-calendar-alt text-theme"></i>
                                                {{ \Carbon\Carbon::parse($popular->created_at)->translatedFormat('d F Y') }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
