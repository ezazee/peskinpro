            ...
            <div class="xl:w-2/3 xl:pr-3 w-full">
                    ...
                    <div class="w-full">
                        ...
                        <div class="list-product-main w-full mt-3">
                            ...
                            <div class="item flex md:mt-7 md:pb-7 mt-5 pb-5 border-b border-line w-full">
                                <div class="w-1/2 flex items-center">
                                    <input type="checkbox" class="product-checkbox mr-2" data-price="{{ $item->productSize->price * $item->quantity - $item->productSize->discount }}" />
                                    <div class="flex items-center gap-6">
                                        <div class="bg-img md:w-[100px] w-20 aspect-[3/4]">
                                            <img src="{{ asset('storage/' . $item->product->front_image) }}" alt="img" class="w-full h-full object-cover rounded-lg">
                                        </div>
                                        <div>
                                ...
                                <div class="w-1/6 flex items-center justify-center">
                                    <div class="quantity-block bg-surface md:p-3 p-2 flex items-center justify-between rounded-lg border border-line md:w-[100px] flex-shrink-0 w-20">
                                        <i class="ph-bold ph-minus cursor-pointer text-base max-md:text-sm" data-id="{{ $item->id }}" id="decrease-quantity"></i>
                                        <div class="text-button quantity">{{ $item->quantity }}</div>
                                        <i class="ph-bold ph-plus cursor-pointer text-base max-md:text-sm" data-id="{{ $item->id }}" id="increase-quantity"></i>
                                    </div>
                                </div>                                                                                  

                                <div class="w-1/6 flex total-price items-center justify-center">
                                    <div class="text-title text-center">Rp{{ number_format($item->productSize->price * $item->quantity - $item->productSize->discount, 0, ',', '.') }}</div>
                                </div>
                ...
                <div class="input-block discount-code w-full h-12 sm:mt-7 mt-5">
                    <form class="w-full h-full relative">
                        <input type="text" placeholder="Tambahkan Diskon Voucher" class="w-full h-full bg-surface pl-4 pr-14 rounded-lg border border-line" />
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    ...
                    if (xhr.status === 200 && response.status === 'success') {
                        const quantityDisplay = document.querySelector(`.quantity[data-id="${cartItemId}"]`);
                        if (quantityDisplay) {
    ...
    const updateTotalSelectedPrice = () => {
        ...
        checkboxes.forEach(checkbox => {
            if (checkbox.checked) {
                ...
                const discount = checkbox.closest('.item').querySelector('.text-secondary2');
                if (discount) {
                    const originalPrice = parseFloat(discount.querySelector('del').textContent.replace(/[^\d]/g, ''));
                    totalDiscount += (originalPrice - itemPrice);
                }
            }
        });
