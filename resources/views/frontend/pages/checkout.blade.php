        ...
        <div class="container">
            <div class="content-main flex max-lg:flex-col-reverse gap-y-10 justify-between">
                    ...
                    @else
                        ...
                        @else
                            ...
                            <div class="address-item rounded-frame relative p-4 mt-7">
                                ...
                                <p class="address-description text-secondary py-2">{{ $item->street }} , {{ $item->city->name }}, {{ $item->province->name }}, Indonesia</p>
                            ...
                            <div class="list-product-checkout">
                                ...
                                <div class="col-span-full select-box">
                                    <form id="shipping-form" action="{{ route('check_ongkir') }}" method="POST">
                                        ...
                                        @else
                                            @foreach ($defaultAddresses as $item)
                                                <input type="text" name="province" id="province" value="{{ $item->province_id }}" hidden>
                                                <input type="text" name="city_destination" id="city_destination" value="{{ $item->city_id }}" hidden>
                                            @endforeach
                                        @endif
                                        @foreach ($cartItems as $cartItem)  
                                            @if ($cartItem->productSize)
                                                <input type="number" name="weight" id="weight" value="{{ $cartItem->productSize->size }}" hidden>
                                            @endif
                                        @endforeach
                                    </form>
                ...
                <div class="right lg:w-5/12">
                    ...
                    <form action="{{ route('processpayment') }}" class="block-button flex flex-col items-center gap-y-4 mt-5" method="POST">
                        @csrf
